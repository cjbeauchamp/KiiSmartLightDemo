...
static void write_state(){
     ...
     fp = fopen("smartlight-state.json", "w");
   
     printf("Writing Command %s ", m_smartlight.power ? "true" : "false");

     fprintf(fp,"{\"brightness\":%i,\"color\":[%i,%i,%i],\"power\":%s}", m_smartlight.brightness, m_smartlight.color[0], m_smartlight.color[1], m_smartlight.color[2], m_smartlight.power ? "true" : "false");
        ...
        KII_THING_IF_WRITER writer)
{
 ...
 printf("Updating Device state from smartlight-state.json\n\n");
    if (fp != NULL) {
        char buf[256];
